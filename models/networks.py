...
from .Unet_At_Bg import UnetAtBgGenerator
from .Unet_At_Bg import UnetAtGenerator
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        ...
        net = UnetAtGenerator(input_nc, output_nc, 8, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif netG == 'unet_at_bg':
        net = UnetAtBgGenerator(input_nc, output_nc, 8, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    else:
...
class VGGPerceptualLoss(nn.Module):
    def __init__(self, gpu_ids, resize=True):
        super(VGGPerceptualLoss, self).__init__()
        self.vgg = Vgg16(gpu_ids)
    ...
    def forward(self, input, target):
        ...
        input_gram = [self.gram(fmap) for fmap in input_features]

        target_features = self.vgg(target)
        target_gram = [self.gram(fmap) for fmap in target_features]
        # print(input_gram[0].shape,target_gram[0].shape)
        style_loss = 0.0
        for j in range(4):
            style_loss += self.loss_l1(input_gram[j], target_gram[j])
...
class L1OLoss(nn.Module):
    def __init__(self, channel_in=1, channel_out=1, stride=1, padding=8):
        ...
        for iOrient in range(self.numKernels):
            ...
            self.kernels.append(GaborKernel)
    ...
    def calOrientationGabor(self, image):
        ...
        for iOrient in range(self.numKernels):
            response = F.conv2d(image, self.kernels[iOrient], stride=self.stride, padding=self.padding)
            temp = response.clone()
            temp[temp < 0] = 0
            if iOrient == 0:
                resTensor = temp
            else:
                resTensor = torch.cat([resTensor, temp], dim=1)
    ...
    def forward(self, fake_image0, gt_image0, input_semantics):
        def rgb2gray(img0):
            img = (img0+1)/2.0*255
        ...
        hair_sketch = input_semantics[:,:1,:,:]
        hair_mask = torch.zeros_like(hair_sketch)
        hair_mask[hair_sketch > -1.0] = 1

        # RGB to gray
        fake_image = rgb2gray(fake_image0)
        gt_image = rgb2gray(gt_image0)

        # cal orientation map with two channels
        orientation_fake = self.calOrientationGabor(fake_image) # [n, 2, h, w]
        orientation_gt = self.calOrientationGabor(gt_image)
        
        orientation_fake = orientation_fake * hair_mask
        orientation_gt = orientation_gt * hair_mask
        # cal L1 loss and the log confidence loss

        '''
        hair_mask_t = ((hair_mask[0].cpu().detach().numpy())+1)/2*255
        hair_mask_t = hair_mask_t.transpose(1,2,0).astype("uint8")
        plt.subplot(1,3,1)
        plt.imshow(hair_mask_t,"gray")
        orientation_fake_t = ((orientation_fake[0,:1].cpu().detach().numpy())+1)/2*255
        orientation_fake_t = orientation_fake_t.transpose(1,2,0).astype("uint8")
        plt.subplot(1,3,2)
        plt.imshow(orientation_fake_t)
        orientation_gt_t = ((orientation_gt[0,:1].cpu().detach().numpy())+1)/2*255
        orientation_gt_t = orientation_gt_t.transpose(1,2,0).astype("uint8")
        plt.subplot(1,3,3)
        plt.imshow(orientation_gt_t)

        plt.show()
        '''

        orient_loss = self.criterion(orientation_fake, orientation_gt.detach())
